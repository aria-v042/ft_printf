...
The function is implemented as a static archive, `libftprintf.a`, which can be linked into future projects — and once validated, integrated directly into Libft.
...
```sh
cc your_file.c -L. -lftprintf -I. -o your_program
```

Or, if the library is in a subdirectory:

```sh
cc your_file.c -Lft_printf -lftprintf -Ift_printf -o your_program
```

### Using alongside Libft

If your project already links against `libft.a`, link both archives:

```sh
cc your_file.c -Llibft -lft -Lft_printf -lftprintf -Ilibft -Ift_printf -o your_program
```
...
| Rule | Effect |
|---|---|
| `make` / `make all` | Compiles `libftprintf.a` (including `libft` if present) |
...
| `%x` | Unsigned integer in lowercase hexadecimal (base 16) |
| `%X` | Unsigned integer in uppercase hexadecimal (base 16) |
...
The entry point walks the format string character by character. When it encounters a `%`, it advances one position and passes the next character to a specifier dispatcher (`parse_specifier`). All other characters are written directly to stdout as-is, and the character count is incremented accordingly.
